...
def get_descriptive_statistics(project_csv: str, results_files: str, company: str):
    projects_df = pd.read_csv(project_csv)

    if os.path.exists(results_files):
    ...
    for _, row in projects_df.iterrows():
        ...
        project_stats = {
            ...
            "size_in_kb": row["size_kb"],

        }

        # Get technologies used
        technologies = get_technologies(name, full_name, company)
...
def get_technologies(name: str, full_name: str, company: str) -> int:
    # Try company-specific directory first
    company_dir = f"../data/{company}/latest_commit"
    global_dir = "../data/projects_last_commit"

    # Possible file naming patterns
    patterns = [
        (company_dir, f"{name}_commit.json"),
        (company_dir, f"{full_name}_commit.json"),
        (global_dir, f"{name}_last_commit.json"),
        (global_dir, f"{full_name}_last_commit.json"),
    ]
    ...
    with open(project_file_path, "r") as f:
        ...
        if "config_data" in project_data:
            technologies = project_data["config_data"].get("concepts", [])
        elif "latest_commit_data" in project_data:
            technologies = project_data["latest_commit_data"]["network_data"].get("concepts", [])
        else:
            technologies = []
        return len(technologies)

def get_commits(input_dir: str, name: str, full_name: str) -> tuple[int, int]:
    contributors_statistics = pd.read_csv(f"../data/{input_dir}/contributor_statistics.csv")
...
def get_contributors(input_dir: str, name: str, full_name: str) -> tuple[int, int]:
    contributors_statistics = pd.read_csv(f"../data/{input_dir}/contributor_statistics.csv")
    for _, row in contributors_statistics.iterrows():
        if row["project_name"] == full_name:
...
if __name__ == "__main__":
    ...
    project_csv = f"../data/{args.input}/{args.input}_projects_final.csv"
    results_files = f"../data/{args.input}/descriptive_statistics_results.json"

    # Uncomment to collect descriptive statistics
    get_descriptive_statistics(project_csv, results_files, args.input)
