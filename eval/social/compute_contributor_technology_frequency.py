...
def process_single_project(
...
) -> Optional[Tuple[str, pd.DataFrame, Dict[str, Dict[str, int]]]]:
    """
    ...
    Returns:
        Tuple of (project_name, frequency_df, contributor_tech_counts) or None on error
    """
    try:
        ...
        return project_name, frequency_df, contributor_tech_counts
...
def process_all_projects(
    ...
    for idx, csv_file in enumerate(csv_files, 1):
        ...
        if result:
            project_name, frequency_df, _ = result
...
def plot_heatmap(frequency_df: pd.DataFrame, project_name: str, output_path: Path,
    ...
    Args:
        ...
        max_contributors: Maximum contributors to show (top by total)
    ...
    # Filter to top contributors and technologies
    ...
    df_filtered = df_filtered.loc[
        df_filtered.sum(axis=1).sort_values(ascending=False).index,
        df_filtered.sum(axis=0).sort_values(ascending=False).index
    ]
...
def main():
    ...
    project_name, frequency_df, contributor_tech_counts = result
    ...
    if not args.no_plot:
        ...
        plot_heatmap(frequency_df, project_name, output_plot)
